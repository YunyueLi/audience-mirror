...
- Human Anchor 校准指标；
- 可运行的本地证据工作台；
- 65 项测试全部通过，包括静态公开包边界、真实 MP4 音画编码／解码、磁盘实验恢复、证据帧调用上限与长片时间分层、人工字幕选择／解析／融合、语义 Timeline 压缩、远程授权门、模型终止语义、Benchmark／稳定性／盲测 Schema、界面系统契约、错版 Human Anchor 拒绝和 Environment／Trace 合同；
...
1. 当前公开版 `v0.2.0-alpha.2` 与待发布 `v0.2.0-alpha.3` 都是本地 MVP／开发者预览，不是人群预测服务；
...
截至同日的下一迭代已建立 `sintel-public-dev-v0.1`：31 道公开开发题、每题时间证据、版本化合同、答案／时间定位／证据分轨评分与 CI。当前只有一位维护者完成草案标注，尚未达到第二位独立人工复核和 `frozen` 状态；因此它可以用于开发与发现失败，不能用于对外发布稳定模型排名。

2026-08-21 的后续运行把无字幕与人工字幕两个条件各重复三次：宏平均分别为 62.4% 与 81.7%，`human_sample_size` 固定为 0；人工字幕提升对白和顺序题，也伴随视觉事实下降。另有一次 Terra 无字幕敏感性探针为 64.5%，不并入 Sol 稳定性。当前结果足以验证评测、重复性和分模态消融链路，不足以证明真人预测或发布稳定模型排名。
